...
const MovieCard = ({
  ...
  return (
    <div className="movie-card bg-dark-100 text-amber-50 dark:bg-gray-800 rounded-2xl shadow-inner shadow-light-100/10 overflow-hidden transition-transform duration-300 hover:scale-105">
      <Link to={`/details/${poster_path ? 'movie' : 'tv'}/${id}`} className="block focus:outline-none" tabIndex={0} aria-label={`View details for ${title || name}`}> 
        ...
        <p>
          ...
          {overview.length > 20 && (
            <span className="text-blue-500 cursor-pointer" onClick={e => { e.preventDefault(); toggleOverview(); }}>
